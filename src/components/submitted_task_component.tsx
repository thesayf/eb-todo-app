...
const SubmittedTaskComponent: React.FC<SubmittedTaskComponentProps> = ({
  task,
  index,
  handleTaskToggle,
  handleAssignmentDisplay,
}) => {
  const {
    attributes,
    listeners,
  ...
  if (isDragging) {
    return (
      <div
        className=" border rounded w-full h-16 bg-base-300 flex flex-row items-center"
        ref={setNodeRef}
        style={style}
        {...attributes}
        {...listeners}
