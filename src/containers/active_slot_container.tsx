...
const defaultTasks = [
  ...
  },
  {
    id: "2",
    columnId: "todo",
    content: "Develop user registration functionality with OTP ",
  },
  {
    id: "3",
    columnId: "doing",
...
const ActiveSlotContainer: React.FC<ActiveSlotContainerProps> = ({ slots }) => {
  ...
  const onDragStart = (event: any) => {
    ...
    if (active.data.current.type === "Column") {
      ...
      setActiveId(active.data.current.slot.id);
    } else if (active.data.current.type === "Task") {
      setActiveSlot(active.data.current.task);
    }
  };
  ...
  return (
      ...
      {slots.map((slot) => {
        return (
          <DragableSlot
            ...
            id={slot.id}
            slot={slot}
            tasks={tasks.filter((task) => task.columnId === slot.id)}
            isActive={handleIsactive(slot.id)}
          />
        );
      })}
      <TimeSlotAdder />
    ...
    //         columns.map((slot) => (
    //             <DragableSlot key={slot.id} id={slot.id} slot={slot} tasks={tasks.filter(task => task.columnId === slot.id)} isActive={handleIsactive(slot.id)}/>
    //         ))
    ...
    //     <DragOverlay>
    //         <DragableSlot id={activeSlot.id} slot={activeSlot} tasks={tasks.filter(task => task.columnId === activeSlot.id)} isActive={handleIsactive(activeSlot.id)} isOverlay={true}  />
